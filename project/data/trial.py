       ...
       # chain_set 
        for line in fh:
            ...
            if line.startswith('SEQRES'): #information about the chains
                ...
                # amino_dict[line.split()[4:]]=line.split()[2]
       ...
       # b=display1()
        if b =='1'or b=='o':
            ...
            if valid==2:
                ...
                if opened == opened.split('/')[-1]:
                    filename2=input('file exit press y/n ' )
                    if filename =='y':
...
#                SORT AND CREAT LISTS OF ONLY NUMBERS
                ls_aa=list(set(aa_num))
                ls_aa.sort()
                # print(ls_aa)
                num_aa=[]
                for aa in ls_aa:
                ...
                num_helix=[]
                for h in helixes.items():
                    num_helix.append(h[1])
                num_sheets=[]
                for p in sheets.items():
                    num_sheets.append(p[1])
#                 PRINT THE RESPECTIVE NUMBERS
                
#                 for k in chains:            
#                     print('{:2s}{:6s} {:10s}'.format('-','Chain',k))            
#                     for num in num_aa:        
#                         print('{:3s}{:22s} {:>4}'.format(' ','Number of amino acids:',num))
#                         for i in num_helix:
#                             print('{:3s}{:22s} {:>4}'.format(' ','Number of helix:' ,i))
#                             for j in num_sheets:
                                
#                                 print('{:3s}{:22s} {:>4}'.format(' ','Number of sheet:',j))
